...
static List schedule_invasic_jobs(resource_offer_msg_t *offer)
{
	...
	List mapped_job_queue = NULL;         // A point to remember is that List is a typedef to list *
	invasic_job_queue_rec_t *invasic_job_queue_rec;
	...
	mapped_job_queue = list_create(_invasic_job_queue_rec_del);

	invasic_job_queue = build_invasic_job_queue(true, false, log_irm_agent);
	sort_invasic_job_queue(invasic_job_queue);

	while ((invasic_job_queue_rec = (invasic_job_queue_rec_t *) list_pop(invasic_job_queue))) {
		job_ptr  = invasic_job_queue_rec->job_ptr;
		xfree(invasic_job_queue_rec);
...
/*
	...
	details->begin_time = job_details_ptr->begin_time;
	...
	details->pn_min_cpus = job_details_ptr->pn_min_cpus;
	details->pn_min_memory = job_details_ptr->pn_min_memory;
	details->pn_min_tmp_disk = job_details_ptr->pn_min_tmp_disk;		
	details->prolog_running = job_details_ptr->prolog_running;
	details->reserved_resources = job_details_ptr->reserved_resources;
	if (job_details_ptr->req_node_bitmap != NULL)
...
/*	   slurm_attr_init(&attr);
	...
	while (!stop_agent_irm) {
		...
		if (!feedback_initialized) {
		   ...
		   slurm_free_resource_offer_msg(msg->data);
		   if (!stop_agent_irm) stop_irm_agent();
		   //stop_feedback_agent();
		   continue;
		}
		...
		if (buf_val == 1) {
		   ...
		   map_jobs2offer = schedule_invasic_jobs(msg->data);
		   unlock_slurmctld(all_locks);
		}
	#endif
		//slurm_free_resource_offer_resp_msg(msg->data);
		slurm_free_resource_offer_msg(msg->data);
		ret_val = send_resource_offer_resp(msg, buf, map_jobs2offer);
                if (ret_val != SLURM_SUCCESS) {
